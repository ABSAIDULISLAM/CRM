...
@section('content')
            ...
            <div class="row align-items-center">
                ...
                <div class="col-md-8 float-end ms-auto">
                    <div class="d-flex title-head">
                        ...
                        <a href="{{route('Office.lead.create')}}" class="btn add-btn" ><i
                            ...
                            <tr>
                                ...
                                <td class="text-end">
                                    <div class="dropdown dropdown-action">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right">
                                            ...
                                            <a class="dropdown-item" href="{{route('Office.lead.convert.client',['id' => Crypt::encrypt($item->id)]) }}"  onclick="return confirm('Aye you sure to Convert This Lead Into CLient ??')"><i class="fas fa-exchange-alt m-r-5"></i>
                                                Convret To Client</a>
                                            @endif
                                            <a class="dropdown-item" href="#" onclick="contactModal({{$item->id}})"><i
                                                    class="fa-regular fa-add m-r-5"></i>
                                                Contact Record</a>
                                            <a class="dropdown-item" href="{{route('Office.lead.edit',['id' => Crypt::encrypt($item->id)]) }}"><i class="fa-solid fa-pencil m-r-5"></i>
                                                Edit</a>
                                ...
                                <td>
                                    <h2 class="table-avatar d-flex align-items-center">
                                        <a href="{{route('Office.lead.view',['id' => Crypt::encrypt($item->id)]) }}" class="company-img">
                                            <img src="{{ asset($item->image ?: 'backend/assets/img/icons/company-icon-10.svg') }}" alt="Company Image">
                                        </a>
                                        <a class="text-success text-bold" href="{{route('Office.lead.view',['id' => Crypt::encrypt($item->id)]) }}" class="profile-split">{{$item->company_name}}</a>
                                    </h2>
                                </td>
                                <td>{{$item->name}}</td>
                                ...
                                <td>
                                    <a href="{{route('Office.lead.status.update',['id' => Crypt::encrypt($item->id)])}}" onclick="return confirm('Aye you sure to change status ??')" class="text-white btn btn-sm badge-{{ $item->status == 'active' ? 'success' : 'danger' }}">
                                        {{ $item->status }}
                                    </a>
                                </td>
    ...
    @includeIf('office-stuff.lead.partial.add-lead')
    @includeIf('office-stuff.lead.partial.contact-record-result')
    @includeIf('office-stuff.lead.partial.next-contact-date')
