...
@section('title', 'office-stuff-create')
...
@section('content')
    ...
    <div class="crms-title row bg-white">
        ...
        <div class="col p-0 text-end">
            <a href="{{ route('Account.office.index') }}" class="btn btn-primary">Manage Account office</a>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-sm-12">
            <form action="{{route('Account.office.store')}}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="row">
                    ...
                    @php
                        $latestLeadOwner = App\Models\OfficeStuff::latest()->first();
                    ...
                    <div class="col-md-6 my-2">
                        <div class="input-block mb-3">
                            <label class="col-form-label" for="name">
                                <h5>Office Stuff Id<span class="text-danger">*</span></h5>
                            </label>
                            <div class="user-icon">
                                <input type="text" name="office_stuff_id" id="office_stuff_id" placeholder="office Stuff Id" value="{{ $invoiceId }}" readonly
                                    class="form-control @error('office_stuff_id') is-invalid border border-danger @enderror" required>
                                @if ($errors->has('lead_owner_id'))
                                    <span class="error text-danger ms-5">{{ $errors->first('office_stuff_id') }}</span>
                                @endif
                            </div>
                        </div>
                    </div>
